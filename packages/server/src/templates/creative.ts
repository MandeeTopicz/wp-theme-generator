...
import { section, sectionHeading, paragraph, button, heroCover, buttonPair, postGrid, wideImage, splitSection, IMAGES, SPACE, TYPE } from './design-system'
...
/**
 * CREATIVE — Asymmetric layouts, large typography, bold visuals.
 * No features section. Hero + asymmetric posts + about + CTA.
 */

function heroSection(s: ColorSlotMap): string {
  return heroCover(s, `
<!-- wp:heading {"level":1,"textAlign":"center","style":{"typography":{"fontSize":"clamp(3rem,8vw,6rem)","fontWeight":"800","letterSpacing":"-0.03em","lineHeight":"1.05"},"spacing":{"margin":{"bottom":"1.5rem"}}}} -->
...
<!-- wp:heading {"textAlign":"center","level":2,"textColor":"${s.accent}","style":{"typography":{"fontSize":"${TYPE.h2}","fontWeight":"${TYPE.h2Weight}"},"spacing":{"margin":{"bottom":"${SPACE.sectionGap}"}}}} -->
<h2 class="has-text-align-center has-${s.accent}-color has-text-color">{{CTA_HEADING}}</h2>
<!-- /wp:heading -->
